...
def load_IBER(log_file, label_file=None, window='session', train_ratio=0.5, split_type='sequential', save_csv=False, window_size=0):
    """ Load Ibercaja structured log into train and test data
    ...
    if log_file.endswith('.csv'):
        if window == 'session':
            ...
            for idx, row in struct_log.iterrows():
                # regex to detect line "------" in txt file
                if re.match(r'(-*)', row['Content']) is None:
        ...
        raise NotImplementedError('load_IBER() only support csv files!')
